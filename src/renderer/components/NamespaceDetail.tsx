...
export default function NamespaceDetail({ namespace: ns }: Props): JSX.Element {
  ...
  const handleViewYAML = async () => {
    ...
    try {
      const content = await getYAML('namespace', ns.metadata.name, false)
      setYaml(content)
    } catch (err) {
      ...
      <div className="px-6 py-5 border-b border-slate-100 dark:border-white/5 bg-white/5 shrink-0">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white font-mono truncate">{ns.metadata.name}</h3>
            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-widest">NAMESPACE</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold outline outline-1 ${phaseColor}`}>
              {phase.toUpperCase()}
            </span>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <section>
              ...
              <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-2xl p-4 space-y-3">
                <InfoRow label="Status" value={phase} />
                <InfoRow label="Created" value={formatAge(ns.metadata.creationTimestamp) + ' ago'} />
                <InfoRow label="UID" value={ns.metadata.uid} mono />
              </div>
            </section>
                  ...
                  {Object.entries(ns.metadata.annotations)
                    ...
                    .map(([k, v]) => (
                      <div key={k} className="flex flex-col gap-2 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                        ...
                        <span className="text-xs font-medium text-slate-300 break-all leading-relaxed font-mono">{v}</span>
                      </div>
                    ))}
                </div>
              </section>
            ...
            <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 dark:border-white/10 bg-white/5 backdrop-blur-xl shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">
                  {yamlLoading ? 'Loading YAML…' : `Edit — ${ns.metadata.name}`}
                </h3>
              </div>
              <button
                type="button"
...
function InfoRow({ label, value, mono }: { label: string, value: string, mono?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-1">
      ...
      <span className={`text-[11px] font-bold text-slate-300 ${mono ? 'font-mono text-[10px]' : ''}`}>{value || '—'}</span>
    </div>
  )
}
