...
function ResourceMap({ label, values }: { label: string; values?: Record<string, string> }) {
...
function LimitItem({ item }: { item: LimitRangeItem }) {
    return (
        <div className="border border-white/5 rounded-xl p-4 bg-white/[0.02]">
            <span className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-wider mb-3 block">{item.type}</span>
            <ResourceMap label="Default" values={item.default} />
            <ResourceMap label="Default Request" values={item.defaultRequest} />
            <ResourceMap label="Min" values={item.min} />
            <ResourceMap label="Max" values={item.max} />
            <ResourceMap label="Max Limit/Request Ratio" values={item.maxLimitRequestRatio} />
        </div>
    )
}
        ...
        <div className="flex flex-col w-full h-full overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h3 className="text-lg font-black text-slate-900 dark:text-white font-mono truncate tracking-tight">{lr.metadata.name}</h3>
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-[0.2em] flex items-center gap-2">
                                ...
                                {lr.metadata.namespace} · LimitRange · {formatAge(lr.metadata.creationTimestamp)} age
                            </p>
                        </div>
                    </div>
                    <button
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
                <div className="px-8 py-6 space-y-8">
                    {limits.length > 0 ? (
                        <div>
                            <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em] mb-4">Limit Rules ({limits.length})</h4>
                            <div className="space-y-3">
                                {limits.map((item, i) => <LimitItem key={i} item={item} />)}
                            </div>
                        </div>
                    ) : (
                        <p className="text-xs text-slate-400">No limit rules defined.</p>
                    )}
                    ...
                    <div>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-2 shadow-sm">
                            <InfoRow label="Namespace" value={lr.metadata.namespace} />
                            <InfoRow label="Created" value={lr.metadata.creationTimestamp} />
                            <InfoRow label="UID" value={<span className="font-mono text-[10px]">{lr.metadata.uid}</span>} />
                        </div>
                    </div>
                </div>
            </div>
