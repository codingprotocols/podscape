...
import type { KubeNetworkPolicy } from '../../../types'
import { formatAge } from '../../../types'
import { useAppStore } from '../../../store'
import { useShallow } from 'zustand/react/shallow'
import YAMLViewer from '../../common/YAMLViewer'
import { ShieldAlert, Fingerprint, ArrowDownLeft, ArrowUpRight, FileCode, X, Activity } from 'lucide-react'
...
export default function NetworkPolicyDetail({ np }: { np: KubeNetworkPolicy }) {
    const { getYAML, applyYAML, refresh, selectedContext } = useAppStore(useShallow(s => ({ getYAML: s.getYAML, applyYAML: s.applyYAML, refresh: s.refresh, selectedContext: s.selectedContext })))
    ...
    const handleViewYAML = async () => {
        ...
        try {
            const content = await getYAML('networkpolicy', np.metadata.name, false, np.metadata.namespace)
            ...
            <div className="px-8 py-7 border-b border-slate-200 dark:border-white/5 bg-white/5 shrink-0">
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                        <div className="w-12 h-12 rounded-2xl bg-rose-500/10 dark:bg-rose-500/20 flex items-center justify-center shrink-0 border border-rose-500/20 ring-4 ring-rose-500/5">
                            <ShieldAlert className="w-6 h-6 text-rose-500" />
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-lg font-black text-slate-900 dark:text-white font-mono truncate tracking-tight">{np.metadata.name}</h3>
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-[0.2em] flex items-center gap-2">
                                ...
                                {np.metadata.namespace} · NetPol · {formatAge(np.metadata.creationTimestamp)} age
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="px-8 py-6 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Ingress</p>
                                <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{ingressRules} <span className="text-[10px] text-slate-400 uppercase">rules</span></span>
                            </div>
                        </div>
                        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Egress</p>
                                <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{egressRules} <span className="text-[10px] text-slate-400 uppercase">rules</span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-2 shadow-sm">
                            <InfoRow label="Pod Selector" value={
                                <div className="flex flex-wrap gap-1.5 pt-1">
                                    {Object.entries(np.spec.podSelector.matchLabels ?? {}).map(([k, v]) => (
                                    ...
                                    {Object.keys(np.spec.podSelector.matchLabels ?? {}).length === 0 && (
                                        <span className="px-2 py-0.5 bg-emerald-500/10 rounded text-[10px] font-bold text-emerald-500 border border-emerald-500/10 uppercase">Selects All Pods</span>
                                    )}
                                </div>
                            } />
                            <InfoRow label="Policy Types" value={
                                <div className="flex gap-2">
                                    {types.map(t => (
                                        <span key={t} className="px-2 py-0.5 bg-slate-100 dark:bg-slate-900 rounded text-[10px] font-bold text-slate-500 uppercase tracking-tight">
                                            {t}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 dark:border-white/10 bg-white/5 backdrop-blur-xl shrink-0">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-800 flex items-center justify-center">
                                    {yamlLoading
                                        ? <div className="w-4 h-4 border-2 border-slate-400 border-t-rose-500 rounded-full animate-spin" />
                                        : <FileCode className="w-4 h-4 text-slate-500" />
                                    }
                                </div>
                                <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">
                                    {yamlLoading ? 'Loading YAML…' : `YAML — ${np.metadata.name}`}
                                </h3>
                            </div>
                            <button
                                type="button"
                            ...
                            {yamlError ? (
                                ...
                                <YAMLViewer editable
                                    ...
                                    onSave={async (updated) => {
                                        ...
                                        if (myId !== yamlFetchIdRef.current || useAppStore.getState().selectedContext !== ctx) return
                                        refresh()
                                        const next = await getYAML('networkpolicy', np.metadata.name, false, np.metadata.namespace)
                                        if (myId !== yamlFetchIdRef.current || useAppStore.getState().selectedContext !== ctx) return
                                        setYaml(next)
                                    }}
                                />
