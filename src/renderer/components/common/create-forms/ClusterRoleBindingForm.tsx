...
import { ClusterRoleBindingFormState, RoleSubject, generateClusterRoleBindingYAML, DNS_LABEL_RE } from './generateYAML'
...
export default function ClusterRoleBindingForm({ onChange }: Props): JSX.Element {
    const namespaces = useAppStore(s => s.namespaces)
    const rowId = useRef(0)
    const [s, setS] = useState<ClusterRoleBindingFormState>({
        name: '',
        roleRefName: '',
    ...
    useEffect(() => {
        if (isValid) onChange(generateClusterRoleBindingYAML(s))
        else onChange('')
    }, [s, onChange, isValid])
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <input value={s.name} onChange={e => setS(p => ({ ...p, name: e.target.value }))} placeholder="my-clusterrolebinding"
            ...
            <div className="border border-white/10 rounded-lg p-3 bg-white/[0.02] space-y-2">
                ...
                <div className="flex gap-2 items-end">
                    <div className="w-28 shrink-0">
                        ...
                        <div className="mt-0.5 px-2 py-1 text-[10px] bg-white/[0.03] border border-white/10 rounded text-slate-500 font-mono">ClusterRole</div>
                    </div>
