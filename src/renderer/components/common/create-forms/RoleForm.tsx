...
import { RoleFormState, PolicyRule, RBAC_VERBS, generateRoleYAML, DNS_LABEL_RE } from './generateYAML'
...
export default function RoleForm({ onChange }: Props): JSX.Element {
    const namespaces = useAppStore(s => s.namespaces)
    const selectedNamespace = useAppStore(s => s.selectedNamespace)
    const rowId = useRef(0)
    const [s, setS] = useState<RoleFormState>({
        name: '',
    ...
    useEffect(() => {
        if (s.name) onChange(generateRoleYAML(s))
        else onChange('')
    }, [s, onChange])
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <input value={s.name} onChange={e => setS(p => ({ ...p, name: e.target.value }))} placeholder="my-role"
