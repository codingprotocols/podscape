...
import type { KubeServiceAccount } from '../types'
import { formatAge } from '../types'
import { useAppStore } from '../store'
import YAMLViewer from './YAMLViewer'
import { User, ShieldCheck, Key, FileCode, X, Activity, Cog } from 'lucide-react'
...
export default function SADetail({ sa }: { sa: KubeServiceAccount }) {
    ...
    const handleViewYAML = async () => {
        ...
        try {
            const content = await getYAML('serviceaccount', sa.metadata.name, false, sa.metadata.namespace)
            setYaml(content)
        } catch (err) {
    ...
    const secrets = sa.secrets?.length ?? 0
    const pullSecrets = sa.imagePullSecrets?.length ?? 0
            ...
            <div className="px-8 py-7 border-b border-slate-200 dark:border-white/5 bg-white/5 shrink-0">
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                        <div className="w-12 h-12 rounded-2xl bg-sky-500/10 dark:bg-sky-500/20 flex items-center justify-center shrink-0 border border-sky-500/20 ring-4 ring-sky-500/5">
                            <User className="w-6 h-6 text-sky-500" />
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-lg font-black text-slate-900 dark:text-white font-mono truncate tracking-tight">{sa.metadata.name}</h3>
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-[0.2em] flex items-center gap-2">
                                ...
                                {sa.metadata.namespace} · ServiceAccount · {formatAge(sa.metadata.creationTimestamp)} age
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="px-8 py-6 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Secrets</p>
                                <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{secrets}</span>
                            </div>
                        </div>
                        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Pull Secrets</p>
                                <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{pullSecrets}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-2 shadow-sm">
                            <InfoRow label="Automount Token" value={sa.automountServiceAccountToken !== false ? 'True' : 'False'} />
                            {sa.secrets && sa.secrets.length > 0 && (
                                <InfoRow label="Attached Secrets" value={
                                    <div className="flex flex-wrap gap-1.5 pt-1">
                                        {sa.secrets.map((s, i) => (
                                            <span key={i} className="px-2 py-0.5 bg-slate-100 dark:bg-white/5 rounded text-[10px] font-mono text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/5">
                                                {s.name}
                                            </span>
                                        ))}
                                    </div>
                        ...
                        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 dark:border-white/10 bg-white/5 backdrop-blur-xl shrink-0">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">
                                    {yamlLoading ? 'Loading YAML…' : `YAML — ${sa.metadata.name}`}
                                </h3>
                            </div>
                            <button
                                type="button"
                            ...
                            {yamlError ? (
                                ...
                                <YAMLViewer
                                    ...
                                    onSave={async (updated) => {
                                        ...
                                        refresh()
                                        const next = await getYAML('serviceaccount', sa.metadata.name, false, sa.metadata.namespace)
                                        setYaml(next)
                                    }}
                                />
                            ) : null}
