...
export function parseMemoryMiB(mem: string): number {
  ...
  if (mem.endsWith('Ki')) return parseOrZero(mem.slice(0, -2)) / 1024
  if (mem.endsWith('Mi')) return parseOrZero(mem.slice(0, -2))
  if (mem.endsWith('Gi')) return parseOrZero(mem.slice(0, -2)) * 1024
  if (mem.endsWith('Ti')) return parseOrZero(mem.slice(0, -2)) * 1024 * 1024
  if (mem.endsWith('k') || mem.endsWith('K')) return parseOrZero(mem.slice(0, -1)) / 1024
  if (mem.endsWith('M')) return parseOrZero(mem.slice(0, -1))
  if (mem.endsWith('G')) return parseOrZero(mem.slice(0, -1)) * 1024
  return parseOrZero(mem) / (1024 * 1024)
}
